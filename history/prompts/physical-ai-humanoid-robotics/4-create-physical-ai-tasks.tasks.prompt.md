...
user: user
command: /sp.tasks
labels: [tasks,physical-ai,robotics,curriculum]
links:
...
```text
You are a senior AI curriculum architect and Spec-Kit Plus task manager.

INPUT:
- constitution.md (enforces rules)
- spec.md (book specification)
- book_plan.md (module/chapter/week roadmap)

YOUR OBJECTIVE:
Generate a COMPLETE `tasks.md` file that breaks down the book creation
into **atomic, actionable tasks** for Claude Code.

The tasks must:
- Be module-aware
- Follow week-by-week progression
- Respect learning outcomes per chapter/week
- Include required tools, platforms, and hardware
- Maintain strict order
- Be directly executable by AI in chapter generation

OUTPUT REQUIREMENTS:
- Output ONE file: tasks.md
...
MANDATORY SECTIONS:
# Global Rules
- Reference constitution.md as non-negotiable
- Tasks must respect module and weekly boundaries
- Tasks must be atomic (one chapter or section per task)
- Tasks must enforce physical AI and hardware constraints
...
# Capstone Module: Autonomous Humanoid System
  - List all tasks in sequence
# Notes
- Tasks must not violate constitution or spec
- Tasks must not skip any weeks or chapters
- Tasks must be actionable by AI without ambiguity

CRITICAL CONSTRAINTS:
- Do NOT generate chapter content
- Do NOT invent new modules, chapters, or weeks
- Strictly enforce constitution.md, spec.md, and book_plan.md
- Tasks must be fully executable in future chapter generation steps

Generate the tasks.md file now.
```
...
- ✅ Impact: Created comprehensive tasks file for Physical AI & Humanoid Robotics book
- 🧪 Tests: N/A
- 📁 Files: Created tasks.md with atomic, actionable tasks for each module/chapter
- 🔁 Next prompts: Ready for implementation with /sp.implement
- 🧠 Reflection: Tasks follow all required sections and respect constitution/spec constraints
