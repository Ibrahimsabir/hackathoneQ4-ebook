...
class IngestionServiceCohere:
    """
    Ingestion service using Cohere for embeddings instead of OpenAI.
    """
    def __init__(self):
        """
        Initialize all required components with Cohere.
        """
        self.crawler = ContentCrawler()
        self.embedding_client = CohereClient()
        self.qdrant_storage = QdrantStorage()
        self.qdrant_storage.initialize_collection(vector_size=1024)  # Cohere embeddings are typically 1024 dimensions
    ...
    async def run_ingestion_pipeline(self) -> bool:
        """
        Execute the complete ingestion pipeline with Cohere embeddings.

        Returns:
            True if successful, False otherwise
        """
        try:
            logger.info("Starting ingestion pipeline with Cohere embeddings...")
            ...
            for chunk in chunks_with_embeddings:
                # Validate embedding dimensions (Cohere embeddings are typically 1024 dimensions)
                if not validate_embedding_dimensions(chunk.embedding, expected_size=1024):
                    logger.warning(f"Invalid embedding dimensions for chunk {chunk.id}: {len(chunk.embedding)}")
                    continue
    ...
    def get_ingestion_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'embedding_model': Config.COHERE_EMBEDDING_MODEL
        }
...
async def main():
    """
    Main function to run the ingestion pipeline with Cohere.
    """
